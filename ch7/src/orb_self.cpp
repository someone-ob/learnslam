...
typedef vector<uint32_t> DescType; // Descriptor type  描述子类型
 ...
 * empty
 */
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors);//函数ComputeORB 输入图像和Fast角点，输出描述子
 ...
 */
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches);//函数BfMatch 输入描述子desc1和描述子desc2，输出匹配对matches
 
int main(int argc, char **argv){

    //-- 读取图像
    Mat first_image = imread("/home/jzh/Code/learnslam/ch7/1.png", 0);//0表示返回一张灰度图 返回第一张图像的灰度图
    Mat second_image = imread("/home/jzh/Code/learnslam/ch7/2.png", 0);//0表示返回一张灰度图 返回第二张图像的灰度图
    assert(first_image .data != nullptr && second_image.data != nullptr); //assert()为断言函数，如果它的条件返回错误，则终止程序执行

 // detect FAST keypoints1 using threshold=40
  chrono::steady_clock::time_point t1 = chrono::steady_clock::now();//计时开始
  vector<cv::KeyPoint> keypoints1;//关键点 第一张图像
  cv::FAST(first_image, keypoints1, 40);//40表示设定的一个灰度阈值   
  vector<DescType> descriptor1;//描述子1
  ComputeORB(first_image, keypoints1, descriptor1);//调用ComputeORB函数
 
  // same for the second
  vector<cv::KeyPoint> keypoints2;//关键点 第二张图像
  vector<DescType> descriptor2;//描述子2
  ...
  chrono::duration<double> time_used = chrono::duration_cast<chrono::duration<double>>(t2 - t1);//计算ORB提取所花费的时间
  cout << "extract ORB cost = " << time_used.count() << " seconds. " << endl;//输出ORB提取所花费的时间

  // find matches
  vector<cv::DMatch> matches;//匹配matches
